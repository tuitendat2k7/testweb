...
app.post('/api/spots', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    // Tọa độ mặc định (Bắt tạm ở cơ sở Ngũ Hành Sơn nếu địa chỉ quá ngõ ngách tìm không ra)
    let finalLat = 15.9688;
    let finalLng = 108.2618;

    // --- BẮT BUỘC 100% DÒ TỌA ĐỘ THEO ĐỊA CHỈ (Bỏ qua số nhập từ Form) ---
    try {
      const geoRes = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(address)}&format=json&limit=1`, {
        headers: { 'User-Agent': 'CocFoodMap/1.0' }
      });
      const geoData = await geoRes.json();
      
      if (geoData && geoData.length > 0) {
        finalLat = parseFloat(geoData[0].lat);
        finalLng = parseFloat(geoData[0].lon); // OpenStreetMap dùng 'lon'
        console.log(`Đã dò ra tọa độ xịn cho ${address}: ${finalLat}, ${finalLng}`);
      }
    } catch (err) {
